...
function raiderAvoidancePlan(targetsFoundWhileForaging){
    for(let i = 0; i < targetsFoundWhileForaging.length; i++){
        if (targetsFoundWhileForaging[i] === "zombie" || targetsFoundWhileForaging[i] === "raider"){
            console.log ("JARVIS has found a " + targetsFoundWhileForaging[i] + " ENGAGING AVOIDANCE ACTION PLAN!!")
        } else {
            console.log("JARVIS has found some " + targetsFoundWhileForaging[i] + "!")
        }
    }
    console.log(targetsFoundWhileForaging.length)
}

raiderAvoidancePlan(["insulin", "oatmeal", "raider", "zombie", "canned peaches"])
